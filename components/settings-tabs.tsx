...
import { Receipt, DollarSign, Globe, Clock } from 'lucide-react'
...
export function SettingsTabs(props: {
  ...
  const getDefaultBusinessHours = () => {
    ...
    if (props.initialSettings.businessHours && typeof props.initialSettings.businessHours === 'object') {
      return { ...defaultHours, ...props.initialSettings.businessHours }
    }
    return defaultHours
  }
  ...
  const [settings, setSettings] = useState(() => ({
    ...
    businessDays: props.initialSettings.businessDays || ['MONDAY', 'TUESDAY', 'WEDNESDAY', 'THURSDAY', 'FRIDAY', 'SATURDAY'],
    currency: props.initialSettings.currency || 'USD',
    currencySymbol: props.initialSettings.currencySymbol || '$',
    timezone: props.initialSettings.timezone || 'America/New_York',
    dateFormat: props.initialSettings.dateFormat || 'MM/DD/YYYY',
  }))
  ...
  const handleCurrencyChange = (value: string) => {
    ...
    setSettings((s) => ({ ...s, currency: value, currencySymbol: selected?.symbol || '$' }))
  }
  ...
  const renderDayRow = (day: { value: string; label: string }) => {
    ...
    const dayHours =
      settings.businessHours[day.value as keyof typeof settings.businessHours] || ({ start: '09:00', end: '18:00' } as any)
    return (
      <div key={day.value} className="flex items-center gap-3 p-2 border rounded-lg bg-white">
        <div className="flex items-center space-x-2 min-w-[85px]">
          <Checkbox
            ...
            onCheckedChange={(checked) => {
              if (checked) {
                setSettings((s) => ({ ...s, businessDays: [...s.businessDays, day.value] }))
              } else {
                setSettings((s) => ({ ...s, businessDays: s.businessDays.filter((d: string) => d !== day.value) }))
              }
            }}
          />
          <Label
            htmlFor={day.value}
            className={`text-sm font-medium cursor-pointer ${isSelected ? '' : 'text-muted-foreground'}`}
          >
            {day.label}
          </Label>
        </div>
          ...
          <div className="flex items-center gap-2 flex-1">
            <Input
              ...
              onChange={(e) =>
                setSettings((s) => ({
                  ...s,
                  businessHours: { ...s.businessHours, [day.value]: { ...dayHours, start: e.target.value } },
                }))
              }
            ...
            <Input
              ...
              onChange={(e) =>
                setSettings((s) => ({
                  ...s,
                  businessHours: { ...s.businessHours, [day.value]: { ...dayHours, end: e.target.value } },
                }))
              }
              className="h-8 text-sm flex-1"
            />
            ...
            <Card className="shadow-sm border-gray-200">
              <CardHeader className="border-b border-gray-200 bg-white py-4">
                ...
                <CardDescription className="text-gray-600">
                  Seat-plan changes, account approvals, and pause/unpause for your business.
                </CardDescription>
              </CardHeader>
              <CardContent className="pt-4">
                <BillingHistoryList items={props.billingHistory} maxHeightClass="max-h-[min(50vh,420px)]" />
              </CardContent>
            </Card>
          </TabsContent>

          <TabsContent value="payments" className="mt-4">
            <SubscriptionPaymentsHistory billingHistory={props.billingHistory} />
          </TabsContent>
        </Tabs>
      </TabsContent>
      ...
      <TabsContent value="hours" className="mt-4">
        <div className="space-y-4">
          <Card className="shadow-sm border-gray-200">
            ...
            <CardContent className="pt-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">{days.map(renderDayRow)}</div>
            </CardContent>
          </Card>
