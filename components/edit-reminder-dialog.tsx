...
export function EditReminderDialog({
  ...
  useEffect(() => {
    ...
    const d = reminder.dueAt instanceof Date ? reminder.dueAt : new Date(reminder.dueAt)
    setDueAt(Number.isNaN(d.getTime()) ? '' : formatForInput(d))
    setLinkedAppointmentId(reminder.appointmentId?.trim() ?? '')
  }, [open, reminder])
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast({ title: 'Saved', description: 'Reminder updated' })
      router.refresh()
      onOpenChange(false)
    } catch (error: unknown) {
      toast({
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[420px]">
        ...
        <div className="space-y-4 pt-2">
          ...
          <div className="space-y-2">
            <Label htmlFor="edit-reminder-due">Date & time</Label>
            <Input
          ...
          {linkedAppointmentId && onViewAppointment ? (
            <Button
              type="button"
              variant="link"
              className="h-auto p-0 text-sm text-amber-800"
          ...
          <div className="space-y-2">
            <Label htmlFor="edit-reminder-notes">Notes (optional)</Label>
            <Textarea
              id="edit-reminder-notes"
              value={notes}
          ...
          <div className="flex justify-end gap-2 pt-2">
            ...
            <Button onClick={handleSave} disabled={isSubmitting || !reminder}>
